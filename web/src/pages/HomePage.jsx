...
function HomePage() {
    ...
    useEffect(() => {
        ...
        function handleScroll() {
            if (!slidingAudioRef.current) return;

            // Start playing if paused
            if (slidingAudioRef.current.paused) {
            slidingAudioRef.current.currentTime = 0;
            slidingAudioRef.current.play();
            }

            // Clear previous stop timer
            if (scrollTimeout) clearTimeout(scrollTimeout);

            // On scroll stop, set timer
            scrollTimeout = setTimeout(() => {
            const audio = slidingAudioRef.current;
            if (!audio) return;

            // When audio ends, pause it and remove event listener
            const onEnded = () => {
                audio.pause();
